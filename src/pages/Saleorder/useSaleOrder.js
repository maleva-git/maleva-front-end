...
import { jobTypeApi } from '../../features/jobType/jobTypeApi';
import { saleOrderInitialState } from './saleOrder.initialState';
...
import { saleOrderApi } from './saleOrderApi';
...
export const useSaleOrder = (companyId, portOptions = [], jobTypeOptions = []) => {
  ...
  useEffect(() => {
    if (jobConfigData?.data?.jobStatusDetails) {
      const newStatusOptions = jobConfigData.data.jobStatusDetails.map(status => ({
        value: status.status,
  ...
  const handleView = () => {
    console.log('View clicked', formData);
  };

  const handleGeneratePO = () => {
    console.log('Generate PO clicked', formData);
  };

  const handleGenerateQuote = () => {
    console.log('Generate Quote clicked', formData);
  };
  ...
  const handleSave = async () => {
    try {
      ...
      console.log('=== SALE ORDER SAVE DEBUG ===');
      console.log('formData.jobCategory:', formData.jobCategory);
      console.log('formData.billType:', formData.billType);
      console.log('formData.saleType:', formData.saleType);
      console.log('formDataForSave.billType:', formDataForSave.billType);
      console.log('formDataForSave.saleType:', formDataForSave.saleType);
      console.log('============================');
      ...
      console.log('SaleOrder Payload:', JSON.stringify(payload, null, 2));
